function renderSettings(root) {
  ...
  root.innerHTML = `
    <div class="space-y-5 max-w-2xl">
      <div>
        <h1 class="display text-3xl" style="color:var(--foreground);">Configuración</h1>
        <p class="text-sm mt-0.5" style="color:var(--muted-foreground);">Usuarios e integraciones del sistema</p>
      </div>

      <!-- Usuario actual -->
      <div class="card">
        <h2 class="font-semibold text-sm mb-4" style="color:var(--foreground);">Tu cuenta</h2>
        <div class="flex items-center gap-4">
          <div class="w-11 h-11 rounded-full flex items-center justify-center text-base font-bold shrink-0 text-white" style="background:var(--primary);">
            ${initial}
          </div>
          <div class="flex-1 min-w-0">
            <p class="font-medium" style="color:var(--foreground);">${user.name || '—'}</p>
            <p class="text-sm truncate" style="color:var(--muted-foreground);">${user.email || '—'}</p>
          </div>
          <span class="badge badge-info">${user.role || 'admin'}</span>
        </div>
      </div>
      ...
      <div class="card">
        <div class="flex items-start justify-between gap-3 mb-4">
          <div>
            <h2 class="font-semibold text-sm" style="color:var(--foreground);">Integraciones · Reuniones</h2>
            <p class="text-xs mt-1" style="color:var(--muted-foreground);">Zoom, Google Meet y WhatsApp para el Agente 03. Sync vía OAuth o webhooks.</p>
          </div>
        ...
        <div id="reuniones-integrations" class="space-y-3">
          <p class="text-sm" style="color:var(--muted-foreground);">Cargando...</p>
        </div>
        <p class="text-xs mt-4 leading-relaxed" style="color:var(--muted-foreground);">
          ...
          <a href="#agent/reuniones" class="underline" style="color:var(--primary);">Agentes → Reuniones</a>
      ...
      <div class="card">
        <div class="flex items-start justify-between gap-3 mb-4">
          <div>
            <h2 class="font-semibold text-sm" style="color:var(--foreground);">Integraciones · Performance</h2>
            <p class="text-xs mt-1" style="color:var(--muted-foreground);">Fuentes de métricas para Agente Performance / Reporting. Stubs hasta conectar env en Vercel.</p>
          </div>
        ...
        <p class="text-xs mt-4 leading-relaxed" style="color:var(--muted-foreground);">
          ...
          <a href="#agent/performance" class="underline" style="color:var(--primary);">Agentes → Performance</a>.
        </p>
      </div>

      <!-- Crear usuario -->
      <div class="card">
        <h2 class="font-semibold text-sm mb-4" style="color:var(--foreground);">Crear usuario admin</h2>
        <div class="space-y-3">
          <div>
          ...
          <div>
            <label class="text-xs font-semibold mb-1.5 block uppercase tracking-wider" style="color:var(--muted-foreground);">Email</label>
            <input id="new-email" type="email" placeholder="email@pampai.com" class="input">
          </div>
          <div>
            <label class="text-xs font-semibold mb-1.5 block uppercase tracking-wider" style="color:var(--muted-foreground);">Contraseña</label>
      ...
      <div class="card">
        <h2 class="font-semibold text-sm mb-4" style="color:var(--foreground);">Integraciones del sistema</h2>
        <div class="space-y-1">
...
function renderIntegration(name, active, note) {
  return `<div class="flex items-center justify-between py-3" style="border-bottom:1px solid var(--secondary);">
    <div class="flex items-center gap-3">
      <div class="w-2 h-2 rounded-full shrink-0" style="background:${active ? 'var(--success)' : 'var(--border)'};"></div>
      <div>
        <p class="text-sm font-medium" style="color:var(--foreground);">${name}</p>
        <p class="text-xs" style="color:var(--muted-foreground);">${note}</p>
      </div>
    </div>
    <span class="text-xs font-semibold px-2.5 py-1 rounded-lg" style="${active
      ? 'background:var(--success-soft);color:var(--success);border:1px solid rgb(43 212 189 / 0.35);'
      : 'background:var(--elevated);color:var(--muted-foreground);border:1px solid var(--border);'}">
...
function statusPill(status) {
  const map = {
    connected: { label: 'Conectado', bg: 'var(--success-soft)', color: 'var(--success)', border: 'rgb(43 212 189 / 0.35)' },
    pending_config: { label: 'Configurar en Vercel', bg: 'var(--warning-soft)', color: 'var(--warning)', border: 'rgb(245 177 76 / 0.35)' },
    disconnected: { label: 'Desconectado', bg: 'var(--elevated)', color: 'var(--muted-foreground)', border: 'var(--border)' },
    error: { label: 'Error', bg: 'var(--danger-soft)', color: '#B91C1C', border: 'rgb(242 114 109 / 0.35)' },
  };
...
function renderMeetingIntegrationCard(integ) {
  ...
  const steps = (integ.setup?.steps || []).map((s) => `<li class="text-xs leading-relaxed" style="color:var(--muted-foreground);">${s}</li>`).join('');

  return `<div class="rounded-lg border p-4" style="border-color:var(--border);background:#FAFAFA;">
    <div class="flex items-start justify-between gap-3">
      <div class="min-w-0">
        <div class="flex items-center gap-2 flex-wrap">
          <p class="text-sm font-semibold" style="color:var(--foreground);">${meta.name}</p>
          ${statusPill(integ.status)}
        </div>
        <p class="text-xs mt-1" style="color:var(--muted-foreground);">${meta.blurb}</p>
        ${integ.external_account ? `<p class="text-xs mt-1" style="color:var(--foreground);">Cuenta: ${integ.external_account}</p>` : ''}
        ${integ.last_sync_at ? `<p class="text-xs mt-0.5" style="color:var(--muted-foreground);">Último sync: ${fmtDate(integ.last_sync_at)}</p>` : ''}
    ...
    <details class="mt-3">
      <summary class="text-xs cursor-pointer font-medium" style="color:var(--primary);">Cómo configurar</summary>
      <ul class="mt-2 space-y-1 list-disc pl-4">${steps}</ul>
      ${webhook ? `<p class="text-xs mt-2 font-data" style="color:var(--muted-foreground);">Webhook: <code>${webhook}</code></p>` : ''}
    </details>` : ''}
  </div>`;
}
...
function renderPerformanceIntegrationCard(integ) {
  ...
  return `<div class="rounded-lg border p-4" style="border-color:var(--border);background:#FAFAFA;">
    <div class="flex items-start justify-between gap-3">
      <div class="min-w-0">
        <div class="flex items-center gap-2 flex-wrap">
          <p class="text-sm font-semibold" style="color:var(--foreground);">${integ.label}</p>
          ${statusPill(integ.status)}
        </div>
        <p class="text-xs mt-1" style="color:var(--muted-foreground);">${integ.blurb || ''}</p>
        <p class="text-xs mt-1.5" style="color:var(--muted-foreground);">Env: ${vars || '—'}</p>
      </div>
      <span class="text-xs font-semibold px-2.5 py-1 rounded-lg shrink-0" style="${ready
        ? 'background:var(--warning-soft);color:var(--warning);border:1px solid rgb(245 177 76 / 0.35);'
        : 'background:var(--elevated);color:var(--muted-foreground);border:1px solid var(--border);'}">
